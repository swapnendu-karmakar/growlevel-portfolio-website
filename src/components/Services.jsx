    ...
    <section id="services" className="py-24 lg:py-32 px-6 lg:px-8">
      <div ref={ref} className="max-w-6xl mx-auto">
        ...
        <div className="mt-5 flex flex-col lg:flex-row lg:items-end lg:justify-between gap-6">
          <h2
            className={`font-display text-4xl sm:text-5xl font-bold leading-tight text-ink max-w-md animate-fade-up delay-100 ${isVisible ? 'visible' : ''
              }`}
          >
            Services built for scale.
          </h2>
          <p
            className={`font-body text-base text-slate-muted max-w-sm animate-fade-up delay-200 ${isVisible ? 'visible' : ''
              }`}
          >
        ...
        <div className="mt-12 grid md:grid-cols-2 gap-5">
          {services.map((service, i) => {
            ...
            return (
              <div
                key={service.title}
                className={`glass-card group rounded-2xl p-7 cursor-default animate-fade-up ${isVisible ? 'visible' : ''
                  }`}
                style={{ transitionDelay: `${(i + 3) * 80}ms` }}
              >
                <div className="flex items-start justify-between mb-5">
                  <div className="w-12 h-12 rounded-xl bg-accent-pale flex items-center justify-center animate-float group-hover:bg-accent/10 transition-colors duration-300">
